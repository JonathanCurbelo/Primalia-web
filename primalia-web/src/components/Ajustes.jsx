...
import { Bell, AlertTriangle, ChevronLeft, ChevronRight, ChevronDown } from 'lucide-react'
...
function PantallaNotificaciones({ onVolver }) {
  const app = useApp()
      ...
      <div className="max-w-md mx-auto px-4 pt-6 pb-24">
        ...
        <Card className="!p-0 divide-y divide-cardBorder overflow-hidden">
          ...
          <button 
            ...
            className="w-full flex items-center justify-between gap-3 px-4 py-3.5 hover:bg-cardElevated transition"
          >
            <div className="flex items-center gap-3">
              <AlertTriangle size={18} className="text-accent" />
              <span className="text-sm text-textPrimary">Límites de gasto por categoría</span>
            </div>
            <ChevronDown size={16} className={`text-textTertiary transition-transform ${expandirLimites ? 'rotate-180' : ''}`} />
          </button>
          
          {expandirLimites && (
            <div className="px-4 py-3.5 bg-cardElevated space-y-3">
              {CATEGORIAS_GASTO.map(cat => (
                <div key={cat.id} className="flex items-center gap-2">
                  ...
                  <input
                    type="number"
                    placeholder="—"
                    className="w-20 text-right bg-bg border border-cardBorder rounded-lg px-2 py-1 text-sm"
              ...
              <p className="text-[11px] text-textTertiary mt-2">Deja vacío para no vigilar esa categoría.</p>
            </div>
          )}
        </Card>
